...
namespace :draft do   
	...
	def make_players
		CSV.foreach("#{Data_Dir}/players.csv", headers: true) do |row|
			Player.new(name: row["Player Name"].strip, position: row["Position"].strip).save!
		end
	end

	def make_teams
		CSV.foreach("#{Data_Dir}/teams.csv", headers: true) do |row|
			Team.new(name: row["Team Name"].strip, division: row["Division"].strip).save!
		end
	end

	def make_order
		CSV.foreach("#{Data_Dir}/order.csv", headers: true) do |row|
